...
try {
    ...
    foreach ($processos as $processo) {
        try {
            ...
            $ultima_atualizacao = formatarDataMySQL($processo['ultima_atualizacao'] ?? '');
            $protocolado_em = formatarDataMySQL($processo['protocolado_em'] ?? '');
            $cpf = $processo['cpf'] ?? '';
            $nome = $processo['nome'] ?? '';
            
            // Registrar mudança de situação se necessário
            if (isset($processosExistentes[$protocolo])) {
                $situacaoAtual = $processosExistentes[$protocolo]['situacao_atual'];
                if ($situacaoAtual !== $situacao) {
                    registrarMudancaSituacao(
                        $conn,
                        $processosExistentes[$protocolo]['id'],
                        $situacaoAtual,
                        $situacao,
                        $id_empresa
                    );
            ...
            if ($temColunaAnterior && $temColunaProtocolado) {
                $stmtProcesso->bind_param('ssssssssi', $protocolo, $servico, $situacao, $situacao, $ultima_atualizacao, $protocolado_em, $cpf, $nome, $id_empresa);
            } elseif ($temColunaProtocolado) {
                $stmtProcesso->bind_param('sssssssi', $protocolo, $servico, $situacao, $ultima_atualizacao, $protocolado_em, $cpf, $nome, $id_empresa);
            } else {
                $stmtProcesso->bind_param('ssssssi', $protocolo, $servico, $situacao, $ultima_atualizacao, $cpf, $nome, $id_empresa);
            }
            
            if (!$stmtProcesso->execute()) {
                throw new Exception("Erro ao processar protocolo {$protocolo}: " . $stmtProcesso->error);
            }
            ...
            $erros[] = [
                'protocolo' => $protocolo ?? 'Desconhecido',
                'erro' => $e->getMessage()
            ];
        }
    }
    ...
    if (empty($erros)) {
        ...
        echo json_encode([
            ...
            'processados' => $processados,
            'erros' => $erros,
            'message' => 'Alguns processos não puderam ser atualizados'
        ]);
    }
    ...
    error_log("Erro fatal em sync.php: " . $e->getMessage());
    http_response_code(500);
    echo json_encode(['error' => 'Erro interno do servidor']);
}
?> 